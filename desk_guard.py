...
async def check_for_intruder(robot, dsg: DeskSecurityGuard):
    ...
    can_see_intruders = did_occur_recently(
        dsg.time_last_observed_intruder, 1.0)
    if not dsg.is_armed:
...
async def desk_security_guard(robot):
    ...
    # offset from initial pose_angle (up to +45 or -45 from this)
    max_pose_angle = 45
    ...
    while True:
        ...
        if (time.time() > time_for_next_patrol) and not dsg.is_investigating_intruder():
            ...
            face_angle = (robot.pose_angle - initial_pose_angle).degrees
            drive_right = (patrol_offset < 0) or (
                (patrol_offset == 0) and (face_angle > 0))
